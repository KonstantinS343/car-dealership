...
class UsernameResetConfirmViewTest(APITestCase, assertions.EmailAssertionsMixin, assertions.StatusCodeAssertionsMixin):
    def setUp(self):
    ...
    def test_post_not_set_new_username_if_user_does_not_exist(self):
        user = create_user()
        data = {
            "uid": djoser.utils.encode_uid(uuid.uuid4()),
            "token": default_token_generator.make_token(user),
            "new_username": "new_username",
        }
    ...
    @override_settings(DJOSER=dict(settings.DJOSER, **{"USERNAME_RESET_CONFIRM_RETYPE": True}))
    ...
    @override_settings(DJOSER=dict(settings.DJOSER, **{"USERNAME_CHANGED_EMAIL_CONFIRMATION": True}))
